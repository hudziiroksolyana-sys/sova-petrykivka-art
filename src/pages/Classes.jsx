...
const classesContent = {
  uk: {
    ...
    workshops: [
      ...
      { id: 3, image: work3, title: "Квіти в стилі Петриківки", intro: "На цьому майстер-класі ви познайомитесь із базовими елементами Петриківського розпису — квітами, крапками та мазками. Особливу увагу приділимо побудові квіткових форм, плавності ліній та поєднанню кольорів.", details: "Заняття допоможе розвинути відчуття ритму, композиції та впевненості у роботі з пензлем. Ви створите власну квіткову композицію в стилі Петриківки та отримаєте задоволення від творчого процесу.", date: "18 серпня 2025", time: "10:00", duration: "2 год", age: "+10", level: "Початковий", price: "900 ₴" },
      { id: 4, image: work4, title: "Ягоди та листя Петриківки", intro: "Майстер-клас присвячений рослинним мотивам Петриківського розпису — ягодам, листю та ритму композиції. Ви навчитесь передавати об’єм, легкість і характерні декоративні форми, які створюють живу та гармонійну роботу.", details: "У теплій творчій атмосфері ви крок за кроком опануєте нові елементи та складете власну композицію, зберігаючи традиційний стиль і водночас додаючи індивідуальність.", date: "20 серпня 2025", time: "17:00", duration: "2 год", age: "+10", level: "Початковий", price: "900 ₴" },
      { id: 5, image: work5, title: "Орнамент і композиція Петриківки", intro: "Цей майстер-клас присвячений основам побудови орнаменту в стилі Петриківки. Ви розглянете різні види мазків, принципи балансу та створення завершеної композиції.", details: "Заняття допоможе зрозуміти, як окремі елементи поєднуються в єдину гармонійну роботу. Ви навчитесь відчувати ритм, симетрію та впевнено будувати орнаментальні форми.", date: "30 серпня 2025", time: "11:00", duration: "2 год", age: "+10", level: "Початковий", price: "1200 ₴" },
    ],
    months: [
                ...
                <article key={item.id} className="classes-more-card home-reveal classes-reveal" data-reveal="scale">
                  <button type="button" className={`classes-more-card-button${index === activeIndex ? " is-active" : ""}`} onClick={() => selectWorkshop(index)} aria-label={item.title}>
                    ...
                    <h3 className="classes-more-name">
                      {(item.moreTitle || item.title).split("\n").map((line, lineIndex, lines) => (
                        <span key={`${item.id}-${lineIndex}`}>{line}{lineIndex < lines.length - 1 ? <br /> : null}</span>
                      ))}
                    </h3>
                    {item.date ? (
                      (() => {
